...
def main(config, seed, name, quick_test=False):
    ...
    savepath = os.path.join("rnn_results", f"{name}_{seed}", "")
    ...
    model, val_losses, train_losses, wcn = trainer.train_RNN(model, train_loader, val_loader, optimizer, loss_fn, conf, device, generator, savepath, quick_test=quick_test)
    ...
    # Save training and validation losses and weight change norms
    np.save(os.path.join(savepath, "train_losses.npy"), np.array(train_losses))

...
mapfile -t CONFIGS < <(printf '%s\n' configs/${DIR}/autocorr*_ktau*_zeroint*_delay*_normal*.yaml \
                                     configs/${DIR}/autocorr*_ktau*_zeroint*_delay*_vanilla*.yaml \
                                     configs/${DIR}/autocorr*_ktau*_zeroint*_delay*_groups*.yaml | sort)
N_SEEDS=30
...
if echo $BASENAME | grep -q 'normal'; then
    ...
    NAME="autocorr${AUTOCORR}_ktau${KERNEL_TAU}_zeroint${ZEROINT}_delay${DELAY}_normal_eta${ETA}_seed${SEED}"
    ...
    NAME="autocorr${AUTOCORR}_ktau${KERNEL_TAU}_zeroint${ZEROINT}_delay${DELAY}_groups${GROUPS_TAU}_prop${PROP}_seed${SEED}"
else
    VANILLA_TAU=$(echo $BASENAME | grep -oP 'vanilla\K[0-9]+\.?[0-9]*')
    NAME="autocorr${AUTOCORR}_ktau${KERNEL_TAU}_zeroint${ZEROINT}_delay${DELAY}_vanilla${VANILLA_TAU}_seed${SEED}"
fi
...
# sbatch submit_expts_final.sh --offset 0 --dir final
# sbatch submit_expts_final.sh --offset 1000 --dir final
# sbatch submit_expts_final.sh --offset 2000 --dir final
# sbatch --array=0-599 submit_expts_final.sh --offset 3000 --dir final 
